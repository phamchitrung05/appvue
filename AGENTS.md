...
- Màn hình sơ đồ bàn (Order/List, `resources/js/pages/apps/ecommerce/order/list/index.vue`): gọi `GET /v1/dining-tables/floor` trả `{zones: [{id, name, tables: [...]}]}` — khu lấy từ bảng `table_zones`, bàn tham chiếu qua `dining_table.zone_id` (cột `area` đã BỎ, migration 2026_08_30_104521 tự chuyển data area cũ → zone). Trạng thái bàn KHÔNG lưu cột mà suy ra ở `DiningTablesController::floor()`: phiên mở (status=open, end_time=null) có đơn → 'occupied', không đơn → 'ordering', có reserved_at → 'reserved', còn lại 'available'; tổng tiền = sum(total) các đơn status != cancelled của phiên hiện tại. Seeder tạo 2 zone (Trong nhà/Ngoài trời) + 14 bàn trong nhà (01..14) + 6 ngoài trời (T01..T06). Model `TableZone` mới có quan hệ `diningTables()` nhưng CHƯA có CRUD API. LƯU Ý: 11 test cũ trong `ApiCrudTest` fail sẵn từ commit 5b21a34 (chưa rõ nguyên nhân, chưa sửa).
