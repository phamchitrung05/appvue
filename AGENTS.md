...
- **Quy ước mutation Pinia (31/08)**: component KHÔNG gọi `useApi` cho create/update/delete — mọi ghi dữ liệu đi qua action của store (`createX`/`updateX`/`removeX`). Store tự vá state ngay sau response: create → `prependRecord` (đầu danh sách), update → `applyRecord` (vá theo id bằng map ra mảng mới), delete → lọc tại chỗ; KHÔNG refetch toàn bộ sau mutation (chỉ nút "Tải lại" dùng `refresh()`). **CẢNH BÁO: handler `@created`/`@saved` ở component KHÔNG được gọi lại `prependRecord`/`applyRecord`** — store action đã vá rồi, gọi thêm là state nhân đôi (UI hiện 2 thẻ dù DB chỉ 1 record — bug đã gặp ở add-table). Dialog/trang chỉ làm form + validate + dựng payload + snackbar, nhận `{ok, record, message}` từ store; parse lỗi dùng chung `resources/js/utils/apiMutation.js` (`isSuccessStatus` + `parseApiError`). Handler cần dữ liệu mới (vd xếp sort_order trong category-list) phải `await nextTick()` trước khi tìm bản ghi vì watcher store → DnD list cần 1 tick. Cột `status` ('available'/'occupied') đã thêm vào `dining_table` (migration 2026_08_31_060457), validate `Rule::in`, frontend `add-table.vue` đọc trực tiếp.
...
- Routes API: `routes/api.php` chỉ dùng `Route::apiResource` cho từng resource (endpoint tuỳ biến `dining-tables/floor` ĐÃ BỎ 31/08 — frontend đọc sơ đồ bàn từ Pinia store thay vì API).
- Màn hình sơ đồ bàn (Order/List hiện là `resources/js/pages/apps/ecommerce/cashier/index.vue`, nav "Thu Ngân" dưới Dashboard Ecommerce; route cũ `/apps/ecommerce/area/add-table` đã XOÁ, order/list redirect về cashier): KHÔNG gọi endpoint floor — khu đọc từ `useTableZoneStore` (`/v1/table-zones`), bàn từ `useDiningTableStore` (`/v1/dining-tables`), gom nhóm theo `dining_table.zone_id` (cột `area` đã BỎ, migration 2026_08_30_104521 tự chuyển data area cũ → zone). Trạng thái bàn đọc cột `status` ('available'/'occupied') của resource — map ở `resources/js/utils/tableStatuses.js`. Trang Order/Add đọc bàn từ `useDiningTableStore` theo `?table=<id>`. Model `TableZone` có quan hệ `diningTables()` + CRUD API `/v1/table-zones` (POST thiếu is_active tự nhận true qua `$attributes` của model). 11 test cũ fail sẵn trong `ApiCrudTest` đã SỬA XONG 31/08 (assert sang đường dẫn envelope `data.*` + code lỗi HOA) — full suite 45/45 pass.
